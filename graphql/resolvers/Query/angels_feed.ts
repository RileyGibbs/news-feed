...
export default async function angels_feed(parent: unknown, {offset}: Args): Promise<FeedRow[]> {
	const feed: FeedRow[] = await db.getAll(
		`
		SELECT "announcements-" || id as type, title, body, NULL as img_url, fellowship, created_ts FROM announcements WHERE fellowship == 'all' OR fellowship == 'angels'
		UNION ALL
